...
log_message() {
    ...
    echo "${timestamp} - ${message}" # Mostrar también en la consola
}
...
clean_kiosk_config() {
    ...
    # Eliminar usuario TPV y su home (si existe)
    ...
    # Limpiar configuraciones de auto-login de LightDM
    ...
    # Eliminar cronjob de monitoreo de Chrome
    # Usar un archivo temporal para la eliminación también
...
if ! command -v google-chrome &> /dev/null; then
    ...
    # Nuevo método para añadir la clave GPG de Chrome para Ubuntu 22.04+ (más seguro)
    wget -q -O - https://dl.google.com/linux/linux_signing_key.pub | sudo gpg --dearmor -o /etc/apt/keyrings/google-chrome.gpg &>> "${TEMP_LOG_FILE}" || error_exit "Fallo al añadir clave GPG de Google Chrome."
...
# Crear el archivo lightdm.conf si no existe con contenido básico
...
# Crear el directorio de autostart si no existe
...
# Script para iniciar Chrome
CHROME_START_SCRIPT="/home/${TPV_USER}/.config/lxsession/LXDE/autostart" # Este es el archivo de autostart para LXDE
...
# Script para monitorear Chrome y reiniciar
MONITOR_SCRIPT="/usr/local/bin/chrome_monitor.sh"
sudo tee "${MONITOR_SCRIPT}" > /dev/null <<EOF
#!/usr/bin/env bash
# Espera 30 segundos para dar tiempo a Chrome a iniciar
sleep 30
...
# Configurar cron para ejecutar el monitoreo cada 5 minutos
log_message "Configurando tarea cron para monitorear Chrome..."

# Verificar y asegurar que el servicio cron esté corriendo
...
# Crear un archivo temporal para la tabla cron de root
CRONTAB_TEMP_FILE="/tmp/root_crontab_temp_$(date +%s)"

# Obtener las entradas existentes del crontab de root, excluyendo nuestro script, y escribir al archivo temporal
# '|| true' al final de la primera parte evita que el script falle si 'crontab -l' no encuentra una tabla cron existente
sudo crontab -l -u root 2>/dev/null | grep -v "${MONITOR_SCRIPT}" > "${CRONTAB_TEMP_FILE}" || true

# Añadir la nueva tarea cron al archivo temporal
echo "*/5 * * * * ${MONITOR_SCRIPT} >> /var/log/chrome_monitor.log 2>&1" >> "${CRONTAB_TEMP_FILE}"

# DEBUGGING: Añadir estas líneas
log_message "DEBUG: Contenido del archivo temporal ${CRONTAB_TEMP_FILE}:"
sudo cat "${CRONTAB_TEMP_FILE}" >> "${TEMP_LOG_FILE}" # Para que aparezca en el log del script
# FIN DEBUGGING

# Cargar el archivo temporal como la nueva tabla cron para el usuario root
